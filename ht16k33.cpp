 ...
 *             Cotribution of 7-segment, 14-segment & matrix display support
...
void HT16K33::clearAll(){
  for (uint8_t s=0; s<16; s++) {setDisplayRaw(s, 0);}
  _i2c_write(HT16K33_DDAP, displayRam,sizeof(displayRam));
} // clearall
...
// To do it on chip a call to "sendLed" is needed
//
uint8_t HT16K33::set14Seg(uint8_t dig, uint8_t cha){ // position 0-7, 0-127
  if (cha>=0 && cha<128 && dig>=0 && dig<8){
    dig = dig*2;
    // reverse the bits using a lookup table
    //    displayRam[dig] = BitReverseTable256[highByte(seg14Chartable[cha])];
    //    displayRam[dig+1] = BitReverseTable256[lowByte(seg14Chartable[cha])];
    displayRam[dig] = lowByte(seg14Chartable[cha]);
    displayRam[dig+1] = highByte(seg14Chartable[cha]);
    return 0;
  } else {
    return 1;
  }
...
/****************************************************************/
// Turn on one 8x8 matrix but only in memory
// To do it on chip a call to "sendLed" is needed
//
uint8_t HT16K33::set8x8Matrix(uint8_t dig, uint8_t cha) {
  if (cha>=0 && cha<128 && dig>=0 && dig<2) {
    //dig = dig*8;
    //dig = 0;
    for (uint8_t row=0; row<8; row++) {
      displayRam[dig+row*2] = matrix8x8Chartable[cha][row];
      }
    return 0;
  } else {
    return 1;
...
/****************************************************************/
// Scroll up 8x8 matrix
//
uint8_t HT16K33::set8x8MatrixScrollUp(uint8_t dig, uint8_t cha, uint8_t speed) {
  if (cha>=0 && cha<128 && dig>=0 && dig<2) {
    for (uint8_t row=0; row<8; row++) {
      //Serial.println(row);
      uint8_t oldrow1 = _i2c_read(dig+2);
      uint8_t oldrow2 = _i2c_read(dig+4);
      uint8_t oldrow3 = _i2c_read(dig+6);
      uint8_t oldrow4 = _i2c_read(dig+8);
      uint8_t oldrow5 = _i2c_read(dig+10);
      uint8_t oldrow6 = _i2c_read(dig+12);
      uint8_t oldrow7 = _i2c_read(dig+14);
      displayRam[dig+0] = oldrow1; //move row 2->4 (digit0) or row 3->5 (digit1) ...dig ->dig+2
      displayRam[dig+2] = oldrow2; //move row 2->4 (digit0) or row 3->5 (digit1) ...dig ->dig+2
      displayRam[dig+4] = oldrow3; //move row 2->4 (digit0) or row 3->5 (digit1) ...dig ->dig+2
      displayRam[dig+6] = oldrow4; //move row 2->4 (digit0) or row 3->5 (digit1) ...dig ->dig+2
      displayRam[dig+8] = oldrow5; //move row 2->4 (digit0) or row 3->5 (digit1) ...dig ->dig+2
      displayRam[dig+10] = oldrow6; //move row 2->4 (digit0) or row 3->5 (digit1) ...dig ->dig+2
      displayRam[dig+12] = oldrow7; //move row 0->2 (digit0) or row 1->3 (digit1) ...dig ->dig+2
      displayRam[dig+14] = matrix8x8Chartable[cha][row]; //print byte bottom row
      sendLed();
      delay(speed); //speed
      }
    return 0;
...
/****************************************************************/
// Scroll down 8x8 matrix
//
uint8_t HT16K33::set8x8MatrixScrollDown(uint8_t dig, uint8_t cha, uint8_t speed) {
  if (cha>=0 && cha<128 && dig>=0 && dig<2) {
    for (int row = 7; row >= 0; row--) {
      //Serial.println(row);
      uint8_t oldrow0 = _i2c_read(dig+0);
      uint8_t oldrow1 = _i2c_read(dig+2);
      uint8_t oldrow2 = _i2c_read(dig+4);
      uint8_t oldrow3 = _i2c_read(dig+6);
      uint8_t oldrow4 = _i2c_read(dig+8);
      uint8_t oldrow5 = _i2c_read(dig+10);
      uint8_t oldrow6 = _i2c_read(dig+12);
      displayRam[dig+0] = matrix8x8Chartable[cha][row]; //print byte bottom row
      displayRam[dig+2] = oldrow0; //move row 0->2 (digit0) or row 1->3 (digit1) ...dig ->dig+2
      displayRam[dig+4] = oldrow1; //move row 2->4 (digit0) or row 3->5 (digit1) ...dig ->dig+2
      displayRam[dig+6] = oldrow2; //move row 2->4 (digit0) or row 3->5 (digit1) ...dig ->dig+2
      displayRam[dig+8] = oldrow3; //move row 2->4 (digit0) or row 3->5 (digit1) ...dig ->dig+2
      displayRam[dig+10] = oldrow4; //move row 2->4 (digit0) or row 3->5 (digit1) ...dig ->dig+2
      displayRam[dig+12] = oldrow5; //move row 2->4 (digit0) or row 3->5 (digit1) ...dig ->dig+2
      displayRam[dig+14] = oldrow6; //move row 2->4 (digit0) or row 3->5 (digit1) ...dig ->dig+2
      sendLed();
      delay(speed); //speed
      }
    return 0;
...
/****************************************************************/
// Write single string to matrix
//
uint8_t HT16K33::set8x8MatrixChar(uint8_t dig, char value) {
if (dig>=0 && dig<2) {
     uint8_t charnum=36;
     if (value ==' ') charnum=36;
     if (value =='+') charnum=36;
...
/****************************************************************/
// set a 7-segment digit and update NOW
//
uint8_t HT16K33::set7SegNow(uint8_t dig, uint8_t cha, boolean dp){
  uint8_t rc;
  rc=set7Seg(dig, cha, dp);
  if (rc==0){
    return sendLed();
  } else {
    return rc;
...
/****************************************************************/
// set a 16-segment digit and update NOW
//
uint8_t HT16K33::set14SegNow(uint8_t dig, uint8_t cha){
  uint8_t rc;
  rc=set14Seg(dig, cha);
  if (rc==0){
    return sendLed();
...
/****************************************************************/
// set a 8x8 matrix and update NOW
//
uint8_t HT16K33::set8x8MatrixNow(uint8_t dig, uint8_t cha){
  uint8_t rc;
  rc=set8x8Matrix(dig, cha);
  if (rc==0){
    return sendLed();
  } else {
    return rc;
